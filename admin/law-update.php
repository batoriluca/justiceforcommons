...
if(isset($_POST['id']) && isset($_POST['action']) && $_POST['action'] == "productDelete"){
    ...
    if ($action1) {
        echo json_encode(['success_msg' => 'Laws Updatess has been deleted']);
        exit();
    }else{
...
if(isset($_POST['id']) && isset($_POST['status']) && isset($_POST['action']) && $_POST['action'] == "updateStatus"){
    ...
    $action=$db->updateData('lawsUpdates', $attorneyData, $attorneyWhere);
    if ($action) {
        echo json_encode(['success_msg' => 'Laws Updatess status updated']);
        exit();
    }else{
        echo json_encode(['error_msg' => 'Something went wrong']);
        exit();
...
<head>
    <title>Laws Updates | Justice For Commons Admin Panel</title>
...
<body x-data="main"
                        ...
                        <nav aria-label="breadcrumb" class="w-full py-1 px-1">
                            <div class="flex space-x-3">
                                ...
                                <li class="flex items-center space-x-1">
                                    ...
                                    <a href="javaScript:;"
                                        class="flex items-center px-3 text-black dark:text-white">Laws Updates</a>
                                </li>
                            </div>
                        </nav>
                    </div>
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4 flex justify-between items-center">
                        <p class="text-sm font-semibold text-black dark:text-white"> Laws Updates</p>
                        <a type="button" href="create-law-update"
    ...
    document.addEventListener("alpine:init", () => {
        Alpine.data('basic', () => ({
            datatable: null,
            init() {
                fetch('apis/getLawsUpdates') // Replace with the correct path to your PHP file
