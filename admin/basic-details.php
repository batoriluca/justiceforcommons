...
$basicDetails = $db->getData("basic_details");

// The Page
$page = "basic-details";

// Edit Product Details
if(isset($_POST['websiteName']) && isset($_POST['websiteTitle'])){

    $errorsArray  = [];
  
    if(empty($_POST['websiteName'])){
        $errorsArray[] = 'Website Name is required';
    }
    if(empty($_POST['websiteTitle'])){
        $errorsArray[] = 'Website Title is required';
    }
    if(empty($_POST['websiteNumber'])){
        $errorsArray[] = 'Website Number is required';
    }
    if(empty($_POST['websiteEmail'])){
        $errorsArray[] = 'Website Email is required';
    }
    if(empty($_POST['address'])){
        $errorsArray[] = 'Address is required';
    }
    if(empty($_POST['description'])){
        $errorsArray[] = 'Description is required';
    }
    // if(empty($_POST['keywords'])){
    //     $errorsArray[] = 'Catergory/Sub Catergory is required';
    // }
    ...
    $websiteName =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['websiteName'])));
    $websiteTitle =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['websiteTitle'])));
    $websiteNumber =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['websiteNumber'])));
    $websiteEmail =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['websiteEmail'])));
    $address =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['address'])));
    $description =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['description'])));

    $id = $basicDetails[0]['id'];
    $updateBy = $admin[0]['username'];

    $updateOn = date("l, jS F Y / h:i A");

    // example usage of the insertData() function
    $basicDetailsData = array(
        'name' => $websiteName,
        'title' => $websiteTitle,
    ...
    $basicDetailsWhere = "id='$id'";

    $result = $db->updateData('basic_details', $basicDetailsData, $basicDetailsWhere);

    if ($result) {
        echo json_encode(['success_msg' => 'Basic details has been saved']);
        exit();
    } else {
...
<head>
    <title>Edit Product Details | Fattakse Admin Panel</title>
...
<body x-data="main"
                        ...
                        <nav aria-label="breadcrumb" class="w-full py-1 px-1">
                            <div class="flex space-x-3">
                                ...
                                <li class="flex items-center">
                                    ...
                                    <a href="javaScript:;"
                                        class="flex items-center px-3 text-black dark:text-white">Edit basic Details</a>
                                </li>
                            </div>
                        </nav>
                    </div>
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4">
                        <p class="text-sm font-semibold text-black dark:text-white">Edit Basic Details</p>
                    </div>
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            ...
                                                            placeholder="Website Name*" id="websiteName"
                                                            value="<?php echo $basicDetails[0]['name'];?>" type="text">
                                                    </label>
                                                </div>
                                                <div class="mb-4">
                                                    ...
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Website Title*" id="WebsiteTitle"
                                                            value="<?php echo $basicDetails[0]['title'];?>" type="text">
                                                    </label>
                                                </div>
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Number*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Website Number*" id="WebsiteNumber"
                                                            value="<?php echo $basicDetails[0]['number'];?>"
                                                            type="number">
                                                    </label>
                                                </div>
                                                <div class="mb-4">
                                                    ...
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Website Email*" id="WebsiteEmail"
                                                            value="<?php echo $basicDetails[0]['email'];?>"
                                                            type="email">
                                                    </label>
                                                </div>
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Address*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                    <textarea rows="3" cols="7" class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Address*"
                                                            id="websiteAddress"><?php echo $basicDetails[0]['address'];?></textarea>
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="basis-full md:basis-1/2 pr-0 md:pr-10">
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Last update by*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            class="form-input w-full bg-white/20 rounded-lg border border-black/10 dark:border-white/10 px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            value="<?php echo $basicDetails[0]['update_by'];?>" disabled
                                                            type="text">
                                                    </label>
                                                </div>
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Last Update On*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            class="form-input w-full bg-white/20 rounded-lg border border-black/10 dark:border-white/10 px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            value="<?php echo $basicDetails[0]['update_on'];?>" disabled
                                                            type="text">
                                                    </label>
                                                </div>
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Keywords*</p>
                                                    <label class="mt-1.5 w-full flex  -space-x-px">
                                                        <textarea rows="6" cols="3" disabled
                                                            class="form-input w-full bg-white/20 rounded-lg border border-black/10 dark:border-white/10 px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Website Keywords*"
                                                            id="keywords"><?php echo $basicDetails[0]['keywords'];?></textarea>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                        <hr class="mb-4 mt-4 border-b">
                                        <div class="flex flex-col md:flex-row">
                                            <div class="basis-full md:basis-2/2 pr-0 md:pr-10">
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Description*</p>
                                                    <label class="mt-1.5 w-full -space-x-px">
                                                        <textarea rows="3" cols="7"
                                                            class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Website Description*"
                                                            id="description"><?php echo $basicDetails[0]['description'];?></textarea>
                                                        <span id="srchars">250</span>Character(s) Remaining
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex pr-0 md:pr-10 pb-4">
                                            <div class="basis-9/12 md:basis-2/12 pr-0">
                                                <button type="button" id="saveBasicData"
    ...
    $(document).ready(function() {
        $("#saveBasicData").click(function(e) {
            ...
            $("textarea.important").each(function() {
                var input = $(this);
                if (input.val() == '') {
                    input.attr('style', 'border-color: red!important;');
                    emptyInputCount = 1;
            ...
            if (emptyInputCount == 0) {
                let getWebsiteName = $("#websiteName").val();
                let getWebsiteTitle = $("#WebsiteTitle").val();
                let getWebsiteNumber = $("#WebsiteNumber").val();
                let getWebsiteEmail = $("#WebsiteEmail").val();
                let getWebsiteAddress = $("#websiteAddress").val();

                let getWebsiteDescription = $("#description").val();

                postObj = {
                    websiteName: getWebsiteName,
                    websiteTitle: getWebsiteTitle,
                    websiteNumber: getWebsiteNumber,
                    websiteEmail: getWebsiteEmail,
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (parseJson.success_msg) {
                            Toastify({
                                text: "Basic details has been updated",
                                duration: 2000,
                                newWindow: true,
                                close: true,
    ...
    // edit product ends
    var maxLength2 = 250;
    var textarea2 = $('textarea#description');
    var remainingChars2 = maxLength2 - textarea2.val().length;

    $('#srchars').text(remainingChars2);
    ...
    textarea2.on('input', function() {
        ...
        if (textLength > maxLength2) {
            ...
            $(this).val(text);
            textLength = maxLength2;
        }

        remainingChars2 = maxLength2 - textLength;
        $('#rchars').text(remainingChars2);
    });
    </script>
</body>
