...
// Add Product 
if(isset($_FILES['galleryAA']) && isset($_POST['attorneyName'])){
    try{
        $errorsArray  = [];
  
        if(empty($_FILES['galleryAA'])){
            $errorsArray[] = 'Attorney Photo is required';
        }
        ...
        $file_name = $_FILES['galleryAA']['name'];
        $file_size = $_FILES['galleryAA']['size'];
        $file_temp = $_FILES['galleryAA']['tmp_name'];
        $file_type = $_FILES['galleryAA']['type'];
        ...
        if (in_array($file_type, $allowed_types)) {
            ...
            if (move_uploaded_file($file_temp, $path)) {
                ...
                $result = $db->insertData('ourAttorneys', $mediaData);
                if ($result) {
                    echo json_encode(['success_msg' => 'Attorney has been uploaded']);
                    exit();
                } else {
...
if(isset($_POST['id']) && isset($_POST['status']) && isset($_POST['action']) && $_POST['action'] == "updateStatus"){
    ...
    $action=$db->updateData('ourAttorneys', $attorneyData, $attorneyWhere);
    if ($action) {
...
<head>
    <title>Our Attorneys | Justice For Commons Admin Panel</title>
...
<body x-data="main"
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4 flex justify-between items-center">
                        <p class="text-sm font-semibold text-black dark:text-white">Our Attorneys</p>
                        <div x-data="modals">
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-show="open" x-transition x-transition.duration.300
                                        ...
                                        <div
                                            ...
                                            <h5 class="font-semibold text-lg">New Our Attorneys</h5>
                                            <button type="button"
                                        ...
                                        <div class="p-3 text-start">
                                            <div class="text-sm text-black dark:text-white">
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Attorney Name*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Attorney Name*" id="attorneyName" type="text">
                                                    </label>
                                                </div>
                                            </div>
                                            ...
                                            <button type="button"
                                                    class="btn inline-flex items-center bg-green-900 align-middle text-zinc-50 hover:bg-green-600 hover:text-black"
                                                    id="addGallery">Add Attorney</button>
                                                <button type="button"
                    ...
                    <div class="flex flex-col gap-7">
                        <div x-data="basic" class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 px-3 py-4 rounded-md ">
                            <div class>
                                <table id="myTableStriped" class="whitespace-nowrap table-striped table-hover table-bordered"></table>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    document.addEventListener("alpine:init", () => {
        Alpine.data('basic', () => ({
            datatable: null,
            init() {
                fetch('apis/getAttorneys') // Replace with the correct path to your PHP file
                    .then(response => response.json())
                    .then(data => {
                        this.datatable = new simpleDatatables.DataTable('#myTableStriped', {
                            data: {
                                headings: ['S. no', 'Name', 'Status', 'Edit'],
                                data: data
                            },
                            sortable: true,
                            searchable: true,
    ...
    $(document).ready(function() {
        $("#addGallery").click(function(e) {
            ...
            let getAttorneyName = $("#attorneyName").val();
            ...
            form_data.append("attorneyName", getAttorneyName);
            form_data.append("galleryAA", file_data);
    ...
    //                         position: 'center',
    //                         icon: 'success',
    //                         title: 'Attorney has been Uploaded',
    ...
    //                         ).html(
    //                             '<i class="fa-solid fa-circle-exclamation"></i><div class="ml-3 text-sm font-medium">' + parseJson.error_msg + '</div>');
    //                     }
    //                 }
    //             }
    //         });
