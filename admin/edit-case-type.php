...
if($_GET['id']){
    $id =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_GET['id'])));
}else{
    header("location: case-type");
}
...
$attorneyData = $db->getData($table1, $where, $orderBy, $limit);

// Edit Product Details
if(isset($_POST['caseTypeName']) && isset($_GET['id'])){

    $errorsArray  = [];
  
    if(empty($_POST['caseTypeName'])){
        $errorsArray[] = 'Case Type Name is required';
    }
    ...
    $caseTypeName =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['caseTypeName'])));
    ...
    $result = $db->updateData('caseType', $caseTypeData, $attorneyWhere);
    if ($result) {
        echo json_encode(['success_msg' => 'Case Type has been saved']);
        exit();
    } else {
...
// Delete Product
if(isset($_GET['id']) && isset($_POST['action']) && $_POST['action'] == "productDelete"){
    
    // example usage of the insertData() function
    $attorneyWhere = "id='$id'";
    $action1=$db->deleteData('caseType', $attorneyWhere);
    
    if ($action1) {
        echo json_encode(['success_msg' => 'Case Type has been deleted']);
        exit();
    }else{
        echo json_encode(['error_msg' => 'Something went wrong']);
        exit();
...
<head>
    <title>Edit Attorneys Details | Justice For Commons Admin Panel</title>
                        ...
                        <nav aria-label="breadcrumb" class="w-full py-1 px-1">
                            <div class="flex space-x-3">
                                ...
                                <li class="flex items-center">
                                    <span class="text-black/40 dark:text-white/40">/</span>
                                    <a href="javaScript:;"
                                        class="flex items-center px-3 text-black dark:text-white">Edit Attorneys
                                        Details</a>
                                </li>
                            </div>
                        </nav>
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4">
                        <p class="text-sm font-semibold text-black dark:text-white">Edit Product Details</p>
                    </div>
                                    ...
                                    <div x-show="activetabwithicon === 'basicDetails'" class>
                                        <div class="flex flex-col md:flex-row">
                                            <div class="basis-full md:basis-1/2 pr-0 md:pr-10">
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Case Type Name*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            ...
                                                            placeholder="Attorney Name*" id="caseTypeName"
                                                            value="<?php echo $attorneyData[0]['name'];?>" type="text">
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="basis-full md:basis-1/2 pr-0 md:pr-10">
                                                <!-- <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">ID*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            ...
                                                            value="<?php echo $attorneyData[0]['id'];?>"
                                                            disabled type="text">
                                                    </label>
                                                </div> -->
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Add On*</p>
                                                    <?php
                                                    $dateTime = new DateTime($attorneyData[0]['add_on']);
                                                    $formattedDate = $dateTime->format('M d, Y (h:i A)');
                                                    ?>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            class="form-input w-full bg-white/20 rounded-lg border border-black/10 dark:border-white/10 px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Add On*" value="<?php echo $formattedDate;?>"
                                                            disabled type="text">
                                                    </label>
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex pr-0 md:pr-10 pb-4">
                                            <div class="basis-9/12 md:basis-2/12 pr-0">
                                                <button type="button" id="saveProductData"
                                                    ...
                                                    <span>Save Case type</span>
                                                </button>
                                            </div>
                                            <div class="basis-3/12 md:basis-2/12 pr-0">
                                                <a href="<?php echo SERVER_HTTP_ADMIN; ?>case-type"
                                                            ...
                                                            <div class="p-3">
                                                                ...
                                                                <div class="flex justify-end items-center mt-8 gap-4">
                                                                    <button type="button"
                                                                        ...
                                                                        onclick="deleteProduct('<?php echo $attorneyData[0]['id'];?>')"
    ...
    $(document).ready(function() {
        $("#saveProductData").click(function(e) {
            ...
            if (emptyInputCount == 0) {
                let getCaseTypeName = $("#caseTypeName").val();
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (parseJson.success_msg) {
                            Toastify({
                                text: "Case Type has been updated",
                                duration: 2000,
                                newWindow: true,
                                close: true,
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                if (parseJson.success_msg) {
                    ...
                    window.location.href = '<?php echo SERVER_HTTP_ADMIN; ?>attorneys';
                } else {
                    if (parseJson.error_msg) {
                        Toastify({
