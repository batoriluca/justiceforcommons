...
if(isset($_POST['titleName']) && isset($_POST['editor1']) && isset($_GET['id'])){

    $errorsArray  = [];
  
    if(empty($_POST['titleName'])){
        $errorsArray[] = 'titleName Name is required';
    }
    if(empty($_POST['editor1'])){
        $errorsArray[] = 'editor1 Name is required';
    }
    ...
    $result = $db->updateData('courtUpdates', $attorneyData, $attorneyWhere);
    if ($result) {
...
if(isset($_GET['id']) && isset($_POST['action']) && $_POST['action'] == "productDelete"){
    ...
    $attorneyWhere = "id='$id'";
    $action1=$db->deleteData('courtUpdates', $attorneyWhere);
    
    if ($action1) {
        echo json_encode(['success_msg' => 'court Updates has been deleted']);
        exit();
    }else{
        echo json_encode(['error_msg' => 'Something went wrong']);
        exit();
...
<head>
    <title>Edit Court Updates Details | Justice For Commons Admin Panel</title>
                        ...
                        <nav aria-label="breadcrumb" class="w-full py-1 px-1">
                            <div class="flex space-x-3">
                                ...
                                <li class="flex items-center space-x-3">
                                    ...
                                    <a href="<?php echo SERVER_HTTP_ADMIN; ?>court-update"
                                ...
                                <li class="flex items-center">
                                    <span class="text-black/40 dark:text-white/40">/</span>
                                    <a href="javaScript:;"
                                        class="flex items-center px-3 text-black dark:text-white">Edit Court Updates
                                        Details</a>
                                </li>
                            </div>
                        </nav>
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4">
                        <p class="text-sm font-semibold text-black dark:text-white">Edit Court Updates Details</p>
                    </div>
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            ...
                                                            placeholder="Title*" id="titleName" type="text" value="<?php echo $attorneyData[0]['title'];?>">
                                                    </label>
                                                </div>
                                            </div>
                                        </div>

                                        <textarea class="mb-4" id="editor1" name="editor1"><?php echo $attorneyData[0]['content'];?></textarea>
    ...
    $(document).ready(function() {
        $("#saveProductDaxxxta").click(function(e) {
            ...
            if (emptyInputCount == 0) {
                ...
                let getEditor1 = CKEDITOR.instances.editor1.getData();
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (parseJson.success_msg) {
                            Toastify({
                                text: "Court Updates has been updated",
                                duration: 2000,
                                newWindow: true,
                                close: true,
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                if (parseJson.success_msg) {
                    ...
                    window.location.href = '<?php echo SERVER_HTTP_ADMIN; ?>court-update';
                } else {
                    if (parseJson.error_msg) {
                        Toastify({
