...
if(isset($_POST['caseTypeName'])){
    ...
    $result = $db->insertData('caseType', $attorneyData);
    if ($result) {
        echo json_encode(['success_msg' => 'Case Type has been Upload']);
        exit();
    } else {
...
if(isset($_POST['id']) && isset($_POST['status']) && isset($_POST['action']) && $_POST['action'] == "updateStatus"){
    ...
    $action=$db->updateData('caseType', $attorneyData, $attorneyWhere);
    if ($action) {
        echo json_encode(['success_msg' => 'Case Type status updated']);
        exit();
    }else{
        echo json_encode(['error_msg' => 'Something went wrong']);
        exit();
...
<head>
    <title>Case Type | Justice For Commons Admin Panel</title>
...
<body x-data="main"
                        ...
                        <nav aria-label="breadcrumb" class="w-full py-1 px-1">
                            <div class="flex space-x-3">
                                ...
                                <li class="flex items-center space-x-1">
                                    ...
                                    <a href="javaScript:;" class="flex items-center px-3 text-black dark:text-white">Case Type</a>
                                </li>
                            </div>
                        </nav>
                    </div>
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4 flex justify-between items-center">
                        <p class="text-sm font-semibold text-black dark:text-white">Case Type</p>
                        <div x-data="modals">
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-show="open" x-transition x-transition.duration.300
                                        ...
                                        <div
                                            ...
                                            <h5 class="font-semibold text-lg">New Case Type</h5>
                                            <button type="button"
                                        ...
                                        <div class="p-3 text-start">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input class="important form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                                            placeholder="Case Type Name*" id="caseTypeName" type="text">
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="flex justify-end items-center mt-8 gap-4">
                                                <button type="button"
                                                    class="btn inline-flex items-center bg-green-900 align-middle text-zinc-50 hover:bg-green-600 hover:text-black"
                                                    onclick="addProduct()">Add Case Type</button>
                                                <button type="button"
    ...
    document.addEventListener("alpine:init", () => {
        Alpine.data('basic', () => ({
            datatable: null,
            init() {
                fetch('apis/getCaseType') // Replace with the correct path to your PHP file
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                if (parseJson.success_msg) {
                    Toastify({
                        text: "Case Type status has been updated",
                        duration: 2000,
                        newWindow: true,
                        close: true,
                ...
                success: function(result) {
                    ...
                    if (parseJson.success_msg) {
                        Swal.fire({
                            position: 'center',
                            icon: 'success',
                            title: 'Case Type has been Uploaded',
