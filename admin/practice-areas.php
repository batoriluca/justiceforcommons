...
// Edit Product Status
if(isset($_POST['id']) && isset($_POST['status']) && isset($_POST['action']) && $_POST['action'] == "updateStatus"){
        
    $id =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['id'])));
    $status =  mysqli_real_escape_string($conn2, htmlspecialchars(trim($_POST['status'])));
    ...
    $action=$db->updateData('practiceAreas', $attorneyData, $attorneyWhere);
    if ($action) {
        echo json_encode(['success_msg' => 'Attorneys status updated']);
        exit();
    }else{
...
// Add Gallery 
if(isset($_FILES['gallery']) && isset($_POST['practiceAreaName']) && isset($_POST['action']) && $_POST['action'] == "galleryAdd"){
    try{
        $errorsArray  = [];
  
        if(empty($_FILES['gallery'])){
            $errorsArray[] = 'Gallery is required';
        }
        ...
        $practiceAreaName = $_POST['practiceAreaName'];
        ...
        $allowed_types = array('image/png', 'image/jpeg', 'image/jpg', 'video/mp4', 'video/quicktime');
        $allowed_types_img = array('image/png', 'image/jpeg', 'image/jpg');

      
        // if ($file_size < 2097152 && in_array($file_type, $allowed_types)) {
        if (in_array($file_type, $allowed_types)) {

            if(in_array($file_type, $allowed_types_img)){
                if($file_size < 2097152){
                }else{
                    echo json_encode(['error_msg' => 'Image file should be 2 MB']);
                    exit();
                }
            }
            ...
            if (move_uploaded_file($file_temp, $path)) {
                ...
                $result = $db->insertData('practiceAreas', $mediaData);
                if ($result) {
                ...
                echo json_encode(['error_msg' => 'Something went wrong with moving the file']);
                exit();
            }
        } else {
            echo json_encode(['error_msg' => 'Invalid file type or size.']);
            exit();
        }
...
// Delete Product
if(isset($_POST['id']) && isset($_POST['action']) && $_POST['action'] == "productDelete"){
    
    // example usage of the insertData() function
    $id = $_POST['id'];
    $attorneyWhere = "id='$id'";
    $action1=$db->deleteData('practiceAreas', $attorneyWhere);
    
    if ($action1) {
        echo json_encode(['success_msg' => 'Practice Areas has been deleted']);
        exit();
    }else{
        echo json_encode(['error_msg' => 'Something went wrong']);
        exit();
...
<head>
    <title>Our Practice Areas | Justice For Commons Admin Panel</title>
...
<body x-data="main"
        ...
        <div class="main-content flex-1 overflow-x-auto">
                        ...
                        <nav aria-label="breadcrumb" class="w-full py-1 px-1">
                            <div class="flex space-x-3">
                                ...
                                <li class="flex items-center space-x-1">
                                    ...
                                    <a href="javaScript:;" class="flex items-center px-3 text-black dark:text-white">Our
                                        Practice Areas</a>
                                </li>
                            </div>
                        </nav>
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4 flex justify-between items-center">
                        <p class="text-sm font-semibold text-black dark:text-white">Our Practice Areas</p>
                        <div x-data="modals">
                            <button type="button" @click="toggle"
                                class="btn inline-flex items-center bg-green-900 align-middle text-zinc-50 hover:bg-green-600 hover:text-black">
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-show="open" x-transition x-transition.duration.300
                                        ...
                                        <div
                                            ...
                                            <h5 class="font-semibold text-lg">New Our Practice Area</h5>
                                            <button type="button"
                                        ...
                                        <div class="p-3 text-start">
                                            <div class="text-sm text-black dark:text-white">
                                                <div class="mb-4">
                                                    <p class="text-sm font-semibold mb-2">Practice Area Name*</p>
                                                    <label class="mt-1.5 flex -space-x-px">
                                                        <input
                                                            ...
                                                            placeholder="Practice Area Name*" id="practiceAreaName"
                                                            type="text">
                                                    </label>
                                                </div>
                                            ...
                                            <div class="flex justify-end items-center mt-8 gap-4">
                                                <button type="button"
                                                    class="btn inline-flex items-center bg-green-900 align-middle text-zinc-50 hover:bg-green-600 hover:text-black"
                                                    id="addGallery">Add Practice Area</button>
                                                <button type="button"
                                                    class="btn bg-white hover:bg-black hover:text-white"
                    ...
                    <div class="flex flex-col gap-7">
                        <div x-data="basic"
                            class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 px-3 py-4 rounded-md ">
                            <div class>
                                <table id="myTableStriped"
                                    class="whitespace-nowrap table-striped table-hover table-bordered"></table>
                            </div>
                        </div>
                    </div>
                </div>
                <footer
                    ...
                    <p class="text-xs text-black/40 dark:text-white/40">&copy; 2023 Justice for Common</p>
                </footer>
            </div>
    ...
    document.addEventListener("alpine:init", () => {
        Alpine.data('basic', () => ({
            datatable: null,
            init() {
                fetch('apis/getPracticeAreas') // Replace with the correct path to your PHP file
                    .then(response => response.json())
                    .then(data => {
                        this.datatable = new simpleDatatables.DataTable('#myTableStriped', {
                            data: {
                                headings: ['S. no', 'Name', 'Img', 'Status', 'Edit'],
                                data: data
                            },
                            sortable: true,
                            searchable: true,
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                if (parseJson.success_msg) {
                    Toastify({
                        text: "Practice Area status has been updated",
                        duration: 2000,
                        newWindow: true,
                        close: true,
                ...
                reader.onload = function(e) {
                    ...
                    if (mediaType === "image") {
                        $("#galleryPerview").addClass("mb-4").html(
                            '<p class="text-sm font-semibold mb-2 text-center">Gallery Preview</p><a id="displayImageClick" href="https://www.fattakse.com/asset/product_img/no-image.jpg" data-fancybox="gallery"><img id="displayImage" src="https://www.fattakse.com/asset/product_img/no-image.jpg" class="rounded-xl m-auto w-24"></a>'
                        );
                    } else {
    ...
    $(document).ready(function() {
        $("#addGallery").click(function(e) {
            ...
            let getPracticeAreaName = $("#practiceAreaName").val();
            ...
            form_data.append("practiceAreaName", getPracticeAreaName);
    ...
    function deleteProduct(id) {
        jQuery.ajax({
            type: 'POST',
            data: 'id=' + id + '&action=productDelete',
            success: function(result) {
                parseJson = JSON.parse(result);
                if (parseJson.success_msg) {
                    Toastify({
                        text: "Practice Area has been deleted",
                        duration: 2000,
                        newWindow: true,
                        close: true,
                        gravity: "top", // `top` or `bottom`
                    ...
                    window.location.href = '<?php echo SERVER_HTTP_ADMIN; ?>practice-areas';
                } else {
                    if (parseJson.error_msg) {
                        Toastify({
                            text: parseJson.error_msg,
